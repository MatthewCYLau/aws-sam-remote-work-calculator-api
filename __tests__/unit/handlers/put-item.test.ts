...
import { sendMessageToSqsHandler } from "../../../src/handlers/send-message-sqs";
...
describe("Test putItemHandler", function () {
  ...
  it("should add id to the SQS queue", async () => {
    ...
    const result = await sendMessageToSqsHandler(event);

    // Compare the result with the expected result
    expect(result.statusCode).toEqual(201);
    expect(JSON.parse(result.body)).toMatchObject({
      MessageId: "5972648d-f5ec-4941-b1bc-1cd890982a22",
    });
